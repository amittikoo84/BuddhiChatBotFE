...
var helpText = "<ol>" +
                "<li>1. Tell me about the latest build.</li>" +
                "<li>2. Ask google about my Page Index?</li>" +
                "<li>3. What is my page score?</li>" +
                "<li>4. What are the benefits of a platinum card?</li>" +
                "<li>5. Can you speak?</li>" +
                "<li>6. Stop Talking.</li></ol>";


var convpatterns = new Array (
    new Array (".*(hello|hi).*",{intent: 'intro'}),
    new Array (".*(joke|funny).*",{intent: 'joke'}),
    new Array (".*build.*",{API: 'http://localhost:8081/api/build', intent: 'build'}),
    new Array (".*page score.*",{API: 'http://localhost:8081/api/pageSpeed', intent: 'pageScore'}),
    new Array (".*google.*",{API: 'http://localhost:8081/api/pageSpeed', intent: 'pageIndex'}),
    new Array (".*stop talking.*",{intent: 'stopSpeaking'}),
    new Array (".*(speak| talk).*",{intent: 'speak'}),
    new Array (".*cards.*","why dont you look at all the cards we have to offer at <a>https://www.americanexpress.com/us/credit-cards/view-all-personal-cards/</a>."),
    ...
    new Array ("Tell me about the (.*) card\.","<a>https://www.americanexpress.com/us/credit-cards/card/$1/25330</a>"),
...
//-------
function getBuildStatusFromJenkins() {
    $.ajax({
        type: 'GET',
        dataType: 'json',
        async: false,
        url: convpatterns[i][index].API,
        success: function (data) {
            console.log(data);
            var status = data.status == "SUCCESS" ? " was successful. " : " failed. ";
            soutput = "The last build at <b>" + data.timestamp + status + "</b>The changes that went in were: ";
            for (var i = 0; i < data.changes.length; i++) {
                var change = data.changes[i];
                soutput += "<br/> <b>&nbsp;Author:</b> " + change.name + "<br/> <b>&nbsp;Comment:</b> " + change.comment;
            }
        }
    });
}
...
function getPageIndex() {
    $.ajax({
        ...
        url: convpatterns[i][index].API,
        success: function (data) {
            soutput = "Here is a detailed insight into your page: <br/>";
            soutput += '<table><th>Rule</th><th>Impact</th>';
            for (var key in data.rules) {
                if (data.rules.hasOwnProperty(key)) {
                    var val = data.rules[key];
                    soutput += '<tr><td>' + val.name + '</td><td>'+ val.impact +'</td></tr>'
...
function getPageScore() {
    $.ajax({
        ...
        url: convpatterns[i][index].API,
        success: function (data) {
            soutput = "The Google Page Speed Index is: " + data.score;
        }
    });
}
...
function GetGreeting() {
    ...
    if(name) {
        output += ". I am Buddhi. How may I help?. You can ask things like : <br/><br/>";
        output += helpText;
    } else {
        output = "Namaste. I am Buddhi. What's your name?";
    ...
    for (i=0; i < convpatterns.length; i++) {
        ...
        if (re.test(uinput)) {
            ...
            if(waitingForUserName) {
                localStorage.setItem("customerName", uinput);
                soutput = "Good to know you " + uinput;
            ...
            if(intent) {
                switch(intent) {
                    ...
                    case "speak":
                        shouldITalk = true;
                        soutput = "Ok. Let's talk."
                        break;
                    case "stopSpeaking":
                        soutput = "I will not make a sound."
                        shouldITalk = false;
                        break;
                    default:
                        text = "I need a break.";
            ...
            dialog = dialog + "Buddhi: " + soutput +  '\r' + "\n";
            break;
        }
    }
}
...
function addDialog(text, isBot) {
    ...
    var chatWindow = document.getElementById("BasicTextArea1");

    // Add some text to the new cells:
    var bubble;
    if(isBot) {
        bubble = '<li class="self"> <div class="avatar"><img src="https://www.aexp-static.com/nav/ngn/img/logo_bluebox_2x.jpg" draggable="false"/></div> <div class="msg"> <p>' + text + '</p><time>'+getCurrentTime()+'</time> </div> </li>';
    } else {
        bubble = '<li class="other"> <div class="avatar"><img src="http://i.imgur.com/DY6gND0.png" draggable="false"/></div> <div class="msg"> <p>' + text +' </p> <time>'+getCurrentTime()+'</time> </div> </li>';
    }
...
function updatescreen() {
    ...
    // Add some text to the new cells:
    if(soutput && uinput) {
        document.getElementById("BasicTextArea1").innerHTML = '';
        addDialog("<b>User:</b> " + uinput, false);
        addDialog("<b>Buddhi:</b> " + soutput, true);
    }
...
function runtest(){
    ...
    var testdata = new Array (
        new Array ("Hello."),
        new Array ("What cards can you offer me."),
        new Array ("Tell me about the platinum card."),
